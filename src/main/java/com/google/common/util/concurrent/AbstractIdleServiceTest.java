...
public class AbstractIdleServiceTest extends TestCase {
	...
	// Functional tests using real thread. We only verify publicly visible
	// state.
	...
	public void testStop_afterStart() {
		...
		assertEquals(Service.State.TERMINATED, service.state());
		// ASSERT.that(service.transitionStates).has().exactly(Service.State.STARTING,
		// Service.State.STOPPING).inOrder();
	}
