...
public class AbstractServiceTest extends TestCase {
	...
	public void testManualServiceStartStop() throws Exception {
		...
		service.notifyStarted(); // usually this would be invoked by another
									// thread
		assertEquals(State.RUNNING, service.state());
		assertTrue(service.isRunning());
		...
		service.notifyStopped(); // usually this would be invoked by another
									// thread
		assertEquals(State.TERMINATED, service.state());
		assertFalse(service.isRunning());
		assertEquals(ImmutableList.of(State.STARTING, State.RUNNING, State.STOPPING, State.TERMINATED),
	...
	/**
	 * This tests for a bug where if {@link Service#stopAsync()} was called
	 * while the service was {@link State#STARTING} more than once, the
	 * {@link Listener#stopping(State)} callback would get called multiple
	 * times.
	 */
	 ...
	 * {@link #notifyStopped} after calling {@link #startAsync} and
	 * {@link #stopAsync}.
	 */
	...
	private class ThreadedService extends AbstractService {
		 ...
		 * trying to confirm that it is running. If the main thread's stop()
		 * call happens before it has the chance, the test will fail. To avoid
		 * this, the main thread calls this method, which waits until the
		...
		public synchronized void stopping(State from) {
			...
			if (from == State.STARTING) {
				try {
					...
					assertTrue(expected.getMessage().equals("Expected the service to be RUNNING, but was STOPPING"));
				}
			}
		...
		public synchronized void terminated(State from) {
			...
			if (from == State.NEW) {
				try {
					...
					assertNull(expected.getCause());
					assertTrue(expected.getMessage().equals("Expected the service to be RUNNING, but was TERMINATED"));
				}
			}
